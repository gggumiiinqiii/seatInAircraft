...
<script setup lang="ts">
  ...
  function getClass(item: itemParams) {
    if (item.isRoad) return { class: '' }
    const statusClassMap: Record<string, string> = {
      empty: item.type === 'first' ? 'firstSeatBox' : 'secondSeatBox',
      booked: 'bookedSeatBox',
      broken: 'brokenSeatBox',
      selected: 'selectedSeatBox',
    }
    return { class: statusClassMap[item.status] || '' }
  }
